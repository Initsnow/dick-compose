...
use deep_compose_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m deep_compose`,
    // i.e, run the `src-tauri/python/deep_compose/__main__.py`
    let py_script = PythonScript::Module("deep_compose".into());
